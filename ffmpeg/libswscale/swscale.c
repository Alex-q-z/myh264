                                  ...
                                  const int dstStride[])
{
    ...
    // printf("++++++++++ c->srcFormat is: %d ++++++++++\n", c->srcFormat);
    // printf("++++++++++ c->dstFormat is: %d ++++++++++\n", c->dstFormat);
    // printf("AV_PIX_FMT_PAL8 is: %d\n", AV_PIX_FMT_PAL8);
    // printf("AV_PIX_FMT_RGB8 is: %d\n", AV_PIX_FMT_RGB8);
    // printf("AV_PIX_FMT_BGR8 is: %d\n", AV_PIX_FMT_BGR8);
    // printf("AV_PIX_FMT_RGB4_BYTE is: %d\n", AV_PIX_FMT_RGB4_BYTE);
    // printf("AV_PIX_FMT_GRAY8 is: %d\n", AV_PIX_FMT_GRAY8);
    // printf("AV_PIX_FMT_GRAY8A is: %d\n", AV_PIX_FMT_GRAY8A);
    // printf("AV_PIX_FMT_BGR4_BYTE is: %d\n", AV_PIX_FMT_BGR4_BYTE);
    // printf("AV_PIX_FMT_BGR32 is: %d\n", AV_PIX_FMT_BGR32);
    // printf("AV_PIX_FMT_RGB24 is: %d\n", AV_PIX_FMT_RGB24);
    // printf("AV_PIX_FMT_BGR24 is: %d\n", AV_PIX_FMT_BGR24);
    // printf("AV_PIX_FMT_BGR32_1 is: %d\n", AV_PIX_FMT_BGR32_1);
    // printf("AV_PIX_FMT_RGB32 is: %d\n", AV_PIX_FMT_RGB32);
...
#define RU (-(int) (0.169 * 224 / 255 * (1 << RGB2YUV_SHIFT) + 0.5))
    ...
    /*
    printf("print src2 Y before swscale\n");
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
            printf("%d ", src2[0][j + i * srcStride2[0]]);
        }
        printf("\n");
    }
    printf("print src2 U before swscale\n");
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            printf("%d ", src2[1][j + i * srcStride2[1]]);
        }
        printf("\n");
    }
    printf("print src2 V before swscale\n");
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            printf("%d ", src2[2][j + i * srcStride2[2]]);
        }
        printf("\n");
    }*/

    /* qizheng: draw the y vs g plots here */
    // // set y values (on x-axis)
    // for (int i = 0; i < 720; i++) {
    //     for (int j = 0; j < 1280; j++) {
    //         src2[0][j + i * srcStride2[0]] = j + i * 1280;
    //     }
    // }
    // // set u, v values to be fixed
    // for (int i = 0; i < 360; i++) {
    //     for (int j = 0; j < 640; j++) {
    //         src2[1][j + i * srcStride2[1]] = 100;
    //     }
    // }
    // for (int i = 0; i < 360; i++) {
    //     for (int j = 0; j < 640; j++) {
    //         src2[2][j + i * srcStride2[2]] = 100;
    //     }
    // }
    
    // qizheng: comment this out!
    ret = c->swscale(c, src2, srcStride2, srcSliceY_internal, srcSliceH, dst2, dstStride2);
    // int y, u, v, r, g, b;
    // for (int i = 0; i < 720; i++) {
    //     for (int j = 0; j < 1280; j++) {
    //         y = src2[0][j + i * srcStride2[0]];
    //         u = src2[1][j/2 + (i/2) * srcStride2[1]];
    //         u = src2[2][j/2 + (i/2) * srcStride2[2]];
    //         r = 1.162 * y + 0.14124 * u + 2.2524 * v - 325.11;
    //         g = 1.145 * y - 0.35028 * u - 0.23917 * v + 56.48;
    //         b = 1.1638 * y + 2.0847 * u + 0.41808 * v - 340.07;
    //         dst2[0][j + i * dstStride2[0]] = r;
    //         dst2[0][j + 1 + i * dstStride2[0]] = g;
    //         dst2[0][j + 2 + i * dstStride2[0]] = b;
    //     }
    // }

    // av_log(c, AV_LOG_INFO, "c->redDither is: %llu\n", c->redDither);
    // exit(0);

    /* directly write YUV and RGB values of current frame to files */
    FILE* y_fptr = fopen("/Users/qizhengzhang/Desktop/x264/yuv_rgb_data/y_values_decoder", "w+");
    FILE* u_fptr = fopen("/Users/qizhengzhang/Desktop/x264/yuv_rgb_data/u_values_decoder", "w+");
    FILE* v_fptr = fopen("/Users/qizhengzhang/Desktop/x264/yuv_rgb_data/v_values_decoder", "w+");
    FILE* rgb_fptr = fopen("/Users/qizhengzhang/Desktop/x264/yuv_rgb_data/rgb_values_decoder", "w+");

    // write y values
    for (int i = 0; i < 720; i++) {
        for (int j = 0; j < 1280; j++) {
            fprintf(y_fptr, "%d ", src2[0][j + i * srcStride2[0]]);
        }
        fprintf(y_fptr, "\n");
    }
    // write u values
    for (int i = 0; i < 360; i++) {
        for (int j = 0; j < 640; j++) {
            fprintf(u_fptr, "%d ", src2[1][j + i * srcStride2[1]]);
        }
        fprintf(u_fptr, "\n");
    }
    // write v values
    for (int i = 0; i < 360; i++) {
        for (int j = 0; j < 640; j++) {
            fprintf(v_fptr, "%d ", src2[2][j + i * srcStride2[2]]);
        }
        fprintf(v_fptr, "\n");
    }
    // write rgb values
    for (int i = 0; i < 720; i++) {
        for (int j = 0; j < 3840; j++) {
            fprintf(rgb_fptr, "%d ", dst2[0][j + i * dstStride2[0]]);
        }
        fprintf(rgb_fptr, "\n");
    }

    // printf("print src2 Y after swscale\n");
    // for (int i = 0; i < 16; i++) {
    //     for (int j = 0; j < 16; j++) {
    //         printf("%d ", src2[0][j + i * srcStride2[0]]);
    //     }
    //     printf("\n");
    // }
    // printf("print src2 U after swscale\n");
    // for (int i = 0; i < 8; i++) {
    //     for (int j = 0; j < 8; j++) {
    //         printf("%d ", src2[1][j + i * srcStride2[1]]);
    //     }
    //     printf("\n");
    // }
    // printf("print src2 V after swscale\n");
    // for (int i = 0; i < 8; i++) {
    //     for (int j = 0; j < 8; j++) {
    //         printf("%d ", src2[2][j + i * srcStride2[2]]);
    //     }
    //     printf("\n");
    // }
    // printf("print dst2 after swscale\n");
    // for (int i = 0; i < 16; i++) {
    //     for (int j = 0; j < 16; j++) {
    //         printf("%d ", dst2[0][j + i * dstStride2[0]]);
    //     }
    //     printf("\n");
    // }
    // LOG;
