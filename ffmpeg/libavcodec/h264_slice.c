...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    const H264Context *h = sl->h264;
    FILE* fptr = fopen("decoder_qp_values", "a+");
    fprintf(fptr, "qp: %d \n", sl->qscale);
    ...
    /* qizheng: my own fields */
    uint8_t *dest_y_ours;
    FILE* decoded_u_values_fptr = fopen("/Users/qizhengzhang/Desktop/x264/decoder_u_values", "w");
    FILE* decoded_u_qps_fptr = fopen("/Users/qizhengzhang/Desktop/x264/decoder_u_qps", "w");
    FILE* decoded_v_qps_fptr = fopen("/Users/qizhengzhang/Desktop/x264/decoder_v_qps", "w");
    FILE* encoder_uv_qps_fptr = fopen("/Users/qizhengzhang/Desktop/x264/empirical_chroma_qp_matrix_file", "r");
    FILE* tmp_fptr = fopen("/Users/qizhengzhang/Desktop/x264/real_u_qp_values", "w");
    FILE* tmp2_fptr = fopen("/Users/qizhengzhang/Desktop/x264/real_y_qp_values", "w");
    // int* uv_qps_matrix = av_malloc(3600);
    // for (int i = 0; i < 3600; i++) {
    //     fscanf(encoder_uv_qps_fptr, "%d ", &uv_qps_matrix[i]);
    // }
    int current_chroma_qp;
    ...
    if (h->ps.pps->cabac) {
        ...
        for (;;) {
            ...
            // qizheng
            // av_log(h->avctx, AV_LOG_INFO, "decoding mb no. %d\n", mb_counter);
            // if (mb_counter > 20)
            //     exit(0);

            // av_log(h->avctx, AV_LOG_INFO, "chroma qp before ff_h264_decode_mb_cabac is %d\n", sl->chroma_qp[0]);
            // note: enter entropy decoding
            // LOG;
            if (sl->mb_x + 1 >= h->mb_width) 
                fscanf(encoder_uv_qps_fptr, "%d \n", &current_chroma_qp);
            else
                fscanf(encoder_uv_qps_fptr, "%d ", &current_chroma_qp);

            ret = ff_h264_decode_mb_cabac(h, sl, current_chroma_qp);

            if (sl->mb_x + 1 >= h->mb_width) 
                fprintf(tmp_fptr, "%d \n", sl->chroma_qp[0]);
            else
                fprintf(tmp_fptr, "%d ", sl->chroma_qp[0]);
            // LOG;
            // av_log(h->avctx, AV_LOG_INFO, "chroma qp after ff_h264_decode_mb_cabac is %d\n", sl->chroma_qp[0]);

            // ADDED: qizheng
            // extract the DCT matrix here
            // if (sl->mb_xy == 0 || sl->mb_xy == 1) {
            //     av_log(h->avctx, AV_LOG_INFO, "printing residual matrix after ff_h264_decode_mb_cabac()\n");
            //     for (int i = 0; i < 16; i++) {
            //         for (int j = 0; j < 48; j++) {
            //             printf("%d ", sl->mb[48*i+j]);
            //         }
            //         printf("\n");
            //     }
            // }
            ...
            // av_log(h->avctx, AV_LOG_INFO, "chroma qp after ff_h264_hl_decode_mb is %d\n", sl->chroma_qp[0]);
            /*
            av_log(h->avctx, AV_LOG_INFO, "we are now in the middle of decode_slice\n");
            av_log(h->avctx, AV_LOG_INFO, "printing reconstructed frame for mb %d\n", sl->mb_xy);
            dest_y_ours = h->cur_pic.f->data[0] + (10 + 10 * sl->linesize) * 16;
            for (int i = 0; i < 16; i++) {
                for (int j = 0; j < 16; j++) {
                    printf("%d ", dest_y_ours[j + i * sl->linesize]);
                }
                printf("\n");
            }
            av_log(h->avctx, AV_LOG_INFO, "end reconstructed frame for mb %d\n", sl->mb_xy);
            */

            // av_log(h->avctx, AV_LOG_INFO, "h->chroma_y_shift is 1\n");

            dest_y_ours = h->cur_pic.f->data[1] +  sl->mb_x * 8 + sl->mb_y * sl->uvlinesize * 8;
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
                    fprintf(decoded_u_values_fptr, "%d ", dest_y_ours[j + i * sl->uvlinesize]);
                }
                fprintf(decoded_u_values_fptr, "\n");
            }

            fprintf(decoded_u_qps_fptr, "%d ", sl->chroma_qp[0]);
            fprintf(decoded_v_qps_fptr, "%d ", sl->chroma_qp[1]);
            fprintf(tmp2_fptr, "%d ", h->cur_pic.qscale_table[mb_counter]);
            if (sl->mb_x + 1 >= h->mb_width) {
                fprintf(decoded_u_qps_fptr, "\n");
                fprintf(decoded_v_qps_fptr, "\n");
                fprintf(tmp2_fptr, "\n");
            }

            // qizheng: print out init_qp in pps header
            // const PPS* pps = (const PPS*)h->ps.pps_list[sl->pps_id]->data;
            // printf("pps->init_qp is: %d\n", pps->init_qp);
            // exit(0);
            ...
            // qizheng: increment counter here
            mb_counter++;
        }
    } else {
        for (;;) {
...
finish:
    ...
    // av_log(h->avctx, AV_LOG_INFO, "we are now at finish\n");
    // av_log(h->avctx, AV_LOG_INFO, "printing reconstructed frame\n");
    // dest_y_ours = h->cur_pic.f->data[1] + (79 + 44 * sl->linesize) * 16;
    // for (int i = 0; i < 16; i++) {
    //     for (int j = 0; j < 16; j++) {
    //         printf("%d ", dest_y_ours[j + i * sl->linesize]);
    //     }
    //     printf("\n");
    // }
    // av_log(h->avctx, AV_LOG_INFO, "end reconstructed frame\n");
