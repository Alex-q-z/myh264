...
{
    //if (h->mb_encode_flag)
    //    x264_log(h, X264_LOG_INFO, "in mb_encode_i16x16, qp is: %d\n", i_qp);
    ...
    //x264_log(h, X264_LOG_INFO, "+++++ printing matrix after dct +++++\n");
    //for (int i = 0; i < 16; i++) {
    //    for (int j = 0; j < 16; j++) {
    //        printf("%d ", dct4x4[i][j]);
    //    }
    ...
    // Qizheng added: store the raw residual matrix here, for later use
    ALIGNED_ARRAY_64( dctcoef, dct4x4_tmp, [16],[16] );
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
            dct4x4_tmp[i][j] = dct4x4[i][j];
        }
    }
    ...
    // Qizheng added: compute errors of residual matrices
    /*
    if (h->mb.mb_counter == 19) {
        x264_log(h, X264_LOG_INFO, "raw\n");
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                printf("%d ", dct4x4_tmp[i][j]);
            }
            printf("\n");
        }
    }
    ...
    if (h->mb.mb_counter == 19) {
        x264_log(h, X264_LOG_INFO, "dequantized\n");
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                printf("%d ", dct4x4[i][j]);
            }
    ...
    /*
    // old version of error computation (incorrect)
    int error_sum = 0;
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
            error_sum += abs(dct4x4_tmp[i][j] - dct4x4[i][j]);
        }
    }
    //h->mb.i_error = error_sum / 256;
    h->mb.i_error = error_sum;
    */

    //if (h->mb_encode_flag) 
    //        x264_log(h, X264_LOG_INFO, "in mb_encode_i16x16, qp is %d, h->mb.i_error is %d\n", h->mb.i_qp, h->mb.i_error);
    //x264_log(h, X264_LOG_INFO, "in mb_encode_i16x16, h->mb.i_error is %d\n", h->mb.i_error);
    ...
    // qizheng: 6/20 version of error computation
    // commented out on 7/7
    /*
    ALIGNED_ARRAY_64( dctcoef, d1,[64] );
    ALIGNED_ARRAY_64( dctcoef, d2,[64] );
    ALIGNED_ARRAY_64( dctcoef, d3,[64] );
    ALIGNED_ARRAY_64( dctcoef, d4,[64] );
    pixel_sub_wxh_mine(d1, 8, &p_src[0], FENC_STRIDE, &p_dst[0], FDEC_STRIDE);
    pixel_sub_wxh_mine(d2, 8, &p_src[8], FENC_STRIDE, &p_dst[8], FDEC_STRIDE);
    pixel_sub_wxh_mine(d3, 8, &p_src[8*FENC_STRIDE+0], FENC_STRIDE, &p_dst[8*FDEC_STRIDE+0], FDEC_STRIDE);
    pixel_sub_wxh_mine(d4, 8, &p_src[8*FENC_STRIDE+8], FENC_STRIDE, &p_dst[8*FDEC_STRIDE+8], FDEC_STRIDE);
    for (int i = 0; i < 64; i++) {
        h->mb.i_error += abs(d1[i]);
        h->mb.i_error += abs(d2[i]);
        h->mb.i_error += abs(d3[i]);
        h->mb.i_error += abs(d4[i]);
    }
    */
}
...
static ALWAYS_INLINE void mb_encode_chroma_internal( x264_t *h, int b_inter, int i_qp, int chroma422 )
{
    ...
    for( int ch = 0; ch < 2; ch++ )
    {
        ...
        for( int i8x8 = 0; i8x8 < (chroma422?2:1); i8x8++ )
        {
            ...
            {
                // x264_log(h, X264_LOG_INFO, "i_qp in encode_chroma is: %d\n", i_qp);
...
static ALWAYS_INLINE void macroblock_encode_internal( x264_t *h, int plane_count, int chroma )
{
    ...
    if( chroma )
    {
        // qizheng: added, get chroma qp here
        //x264_log(h, X264_LOG_INFO, "before predict_chroma, h->mb.i_chroma_qp is: %d\n", h->mb.i_chroma_qp);
        //h->mb.i_chroma_qp = 1;
        ...
        // qizheng: added, get chroma qp here
        //x264_log(h, X264_LOG_INFO, "before encode_chroma, h->mb.i_chroma_qp is: %d\n", h->mb.i_chroma_qp);
