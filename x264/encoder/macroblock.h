...
static inline void pixel_sub_wxh_mine( dctcoef *diff, int i_size,
...
{
    //if (h->mb_encode_flag)
    //    x264_log(h, X264_LOG_INFO, "in mb_encode_i4x4, qp is: %d\n", i_qp);
    ...
    ALIGNED_ARRAY_64( dctcoef, dct4x4_tmp,[16] );
    for (int i = 0; i < 16; i++) {
        dct4x4_tmp[i] = dct4x4[i];
    }
    ...
    if( nz )
    {
        ...
        int error_sum = 0;
        for (int i = 0; i < 16; i++) {
            error_sum += abs(dct4x4_tmp[i] - dct4x4[i]);
        }
        //h->mb.i_error = error_sum / 16;
        h->mb.i_error = error_sum;

        x264_log(h, X264_LOG_INFO, "error_sum is %d", error_sum);
        */

        /*
        x264_log(h, X264_LOG_INFO, "printing dct4x4_tmp\n");
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                printf("%d ", dct4x4_tmp[i*4+j]);
            }
            printf("\n");
        }

        x264_log(h, X264_LOG_INFO, "printing dct4x4\n");
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                printf("%d ", dct4x4[i*4+j]);
            }
            printf("\n");
        }*/

        //if (h->mb_encode_flag) 
        //    x264_log(h, X264_LOG_INFO, "in mb_encode_i4x4, qp is %d, h->mb.i_error is %d\n", h->mb.i_qp, h->mb.i_error);
        //x264_log(h, X264_LOG_INFO, "in mb_encode_i4x4, h->mb.i_error is %d\n", h->mb.i_error);
    ...
    /*
    ALIGNED_ARRAY_64( dctcoef, d,[16] );
    pixel_sub_wxh_mine( d, 4, p_src, FENC_STRIDE, p_dst, FDEC_STRIDE );
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            h->mb.i_error += abs(d[i*4+j]);
        }
    }
    */
}
...
{
    //if (h->mb_encode_flag)
    //    x264_log(h, X264_LOG_INFO, "in mb_encode_i8x8, qp is: %d\n", i_qp);
    ...
    // Qizheng added: store the raw residual matrix here, for later use
    ALIGNED_ARRAY_64( dctcoef, dct8x8_tmp,[64] );
    for (int i = 0; i < 64; i++) {
        dct8x8_tmp[i] = dct8x8[i];
    }
    ...
    if( nz )
    {
        ...
        int error_sum = 0;
        for (int i = 0; i < 64; i++) {
            error_sum += abs(dct8x8_tmp[i] - dct8x8[i]);
        }
        //h->mb.i_error = error_sum / 64;
        h->mb.i_error = error_sum;
        */

        //if (h->mb_encode_flag) 
        //    x264_log(h, X264_LOG_INFO, "in mb_encode_i8x8, qp is %d, h->mb.i_error is %d\n", h->mb.i_qp, h->mb.i_error);
        /*
        x264_log(h, X264_LOG_INFO, "in mb_encode_i8x8, h->mb.i_error is %d\n", h->mb.i_error);

        x264_log(h, X264_LOG_INFO, "in mb_encode_i8x8, dct8x8_tmp is\n");
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                printf("%d ", dct8x8_tmp[8*i+j]);
            }
            printf("\n");
        }

        x264_log(h, X264_LOG_INFO, "in mb_encode_i8x8, dct8x8 is\n");
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                printf("%d ", dct8x8[8*i+j]);
            }
            printf("\n");
        }
        */
    ...
    // commented out on 7/7
    /*
    ALIGNED_ARRAY_64( dctcoef, d,[64] );
    pixel_sub_wxh_mine( d, 8, p_src, FENC_STRIDE, p_dst, FDEC_STRIDE );
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            h->mb.i_error += abs(d[i*8+j]);
        }
    }
    */
}
