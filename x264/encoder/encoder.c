...
#define LOG printf("Log [File:%s, Func:%s, Line:%d]\n",__FILE__,__func__,__LINE__)
...
#define MAX_RESOLUTION 16384
    ...
    // qizheng: manually modify rc parameters here
...
/*
static void read_qp_matrix(x264_t *h) {
    FILE* qp_matrix_fptr = fopen("/Users/qizhengzhang/Desktop/x264/qp_matrix_file", "r");
    h->qp_matrix = x264_malloc(3600);
    for (int i = 0; i < 3600; i++) {
        fscanf(qp_matrix_fptr, "%d,", &h->qp_matrix[i]);
    }
    return ;
}*/

// static void read_error_bound_matrix(x264_t *h) {
//     FILE* error_bound_matrix_fptr = fopen("/Users/qizhengzhang/Desktop/x264/error_bound_matrix_file", "r");
//     h->error_bound_matrix = x264_malloc(3600);
//     for (int i = 0; i < 3600; i++) {
//         fscanf(error_bound_matrix_fptr, "%d,", &h->error_bound_matrix[i]);
//     }
//     return ;
// }

static inline void pixel_sub_wxh_mine_encoder( dctcoef *diff, int i_size,
...
static int compute_y_error( x264_t *h ) {
    int y_error = 0;
    pixel *p_src = h->mb.pic.p_fenc[0];
    pixel *p_dst = h->mb.pic.p_fdec[0];
    ALIGNED_ARRAY_64( dctcoef, d1,[64] );
    ALIGNED_ARRAY_64( dctcoef, d2,[64] );
    ALIGNED_ARRAY_64( dctcoef, d3,[64] );
    ALIGNED_ARRAY_64( dctcoef, d4,[64] );
    pixel_sub_wxh_mine_encoder(d1, 8, &p_src[0], FENC_STRIDE, &p_dst[0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(d2, 8, &p_src[8], FENC_STRIDE, &p_dst[8], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(d3, 8, &p_src[8*FENC_STRIDE+0], FENC_STRIDE, &p_dst[8*FDEC_STRIDE+0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(d4, 8, &p_src[8*FENC_STRIDE+8], FENC_STRIDE, &p_dst[8*FDEC_STRIDE+8], FDEC_STRIDE);
    for (int i = 0; i < 64; i++) {
        y_error += abs(d1[i]);
        y_error += abs(d2[i]);
        y_error += abs(d3[i]);
...
static int compute_u_error( x264_t *h ) {
    int u_error = 0;
    pixel *p_src = h->mb.pic.p_fenc[1];
    pixel *p_dst = h->mb.pic.p_fdec[1];
    ALIGNED_ARRAY_64( dctcoef, d1,[16] );
    ALIGNED_ARRAY_64( dctcoef, d2,[16] );
    ALIGNED_ARRAY_64( dctcoef, d3,[16] );
    ALIGNED_ARRAY_64( dctcoef, d4,[16] );
    pixel_sub_wxh_mine_encoder(d1, 4, &p_src[0], FENC_STRIDE, &p_dst[0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(d2, 4, &p_src[4], FENC_STRIDE, &p_dst[4], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(d3, 4, &p_src[4*FENC_STRIDE+0], FENC_STRIDE, &p_dst[4*FDEC_STRIDE+0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(d4, 4, &p_src[4*FENC_STRIDE+4], FENC_STRIDE, &p_dst[4*FDEC_STRIDE+4], FDEC_STRIDE);
    for (int i = 0; i < 16; i++) {
        u_error += abs(d1[i]);
        u_error += abs(d2[i]);
        u_error += abs(d3[i]);
...
static int compute_v_error( x264_t *h ) {
    int v_error = 0;
    pixel *p_src = h->mb.pic.p_fenc[2];
    pixel *p_dst = h->mb.pic.p_fdec[2];
    ALIGNED_ARRAY_64( dctcoef, d1,[16] );
    ALIGNED_ARRAY_64( dctcoef, d2,[16] );
    ALIGNED_ARRAY_64( dctcoef, d3,[16] );
    ALIGNED_ARRAY_64( dctcoef, d4,[16] );
...
static int compute_rgb_error( x264_t *h ) {
    float rgb_error = 0.0;
    /* y channel */
    pixel *p_src = h->mb.pic.p_fenc[0];
    pixel *p_dst = h->mb.pic.p_fdec[0];
    ALIGNED_ARRAY_64( dctcoef, dy1,[64] );
    ALIGNED_ARRAY_64( dctcoef, dy2,[64] );
    ALIGNED_ARRAY_64( dctcoef, dy3,[64] );
    ALIGNED_ARRAY_64( dctcoef, dy4,[64] );
    pixel_sub_wxh_mine_encoder(dy1, 8, &p_src[0], FENC_STRIDE, &p_dst[0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(dy2, 8, &p_src[8], FENC_STRIDE, &p_dst[8], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(dy3, 8, &p_src[8*FENC_STRIDE+0], FENC_STRIDE, &p_dst[8*FDEC_STRIDE+0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(dy4, 8, &p_src[8*FENC_STRIDE+8], FENC_STRIDE, &p_dst[8*FDEC_STRIDE+8], FDEC_STRIDE);
    for (int i = 0; i < 64; i++) {
        rgb_error += 3.4707 * abs(dy1[i]);
        rgb_error += 3.4707 * abs(dy2[i]);
        rgb_error += 3.4707 * abs(dy3[i]);
        rgb_error += 3.4707 * abs(dy4[i]);
    }
    /* u channel */
    p_src = h->mb.pic.p_fenc[1];
    p_dst = h->mb.pic.p_fdec[1];
    ALIGNED_ARRAY_64( dctcoef, du1,[16] );
    ALIGNED_ARRAY_64( dctcoef, du2,[16] );
    ALIGNED_ARRAY_64( dctcoef, du3,[16] );
    ALIGNED_ARRAY_64( dctcoef, du4,[16] );
    pixel_sub_wxh_mine_encoder(du1, 4, &p_src[0], FENC_STRIDE, &p_dst[0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(du2, 4, &p_src[4], FENC_STRIDE, &p_dst[4], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(du3, 4, &p_src[4*FENC_STRIDE+0], FENC_STRIDE, &p_dst[4*FDEC_STRIDE+0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(du4, 4, &p_src[4*FENC_STRIDE+4], FENC_STRIDE, &p_dst[4*FDEC_STRIDE+4], FDEC_STRIDE);
    for (int i = 0; i < 16; i++) {
        rgb_error += 1.87566 * abs(du1[i]);
        rgb_error += 1.87566 * abs(du2[i]);
        rgb_error += 1.87566 * abs(du3[i]);
        rgb_error += 1.87566 * abs(du4[i]);
    }
    /* v channel */
    p_src = h->mb.pic.p_fenc[2];
    p_dst = h->mb.pic.p_fdec[2];
    ALIGNED_ARRAY_64( dctcoef, dv1,[16] );
    ALIGNED_ARRAY_64( dctcoef, dv2,[16] );
    ALIGNED_ARRAY_64( dctcoef, dv3,[16] );
    ALIGNED_ARRAY_64( dctcoef, dv4,[16] );
    pixel_sub_wxh_mine_encoder(dv1, 4, &p_src[0], FENC_STRIDE, &p_dst[0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(dv2, 4, &p_src[4], FENC_STRIDE, &p_dst[4], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(dv3, 4, &p_src[4*FENC_STRIDE+0], FENC_STRIDE, &p_dst[4*FDEC_STRIDE+0], FDEC_STRIDE);
    pixel_sub_wxh_mine_encoder(dv4, 4, &p_src[4*FENC_STRIDE+4], FENC_STRIDE, &p_dst[4*FDEC_STRIDE+4], FDEC_STRIDE);
    for (int i = 0; i < 16; i++) {
        rgb_error += 2.43131 * abs(dv1[i]);
        rgb_error += 2.43131 * abs(dv2[i]);
        rgb_error += 2.43131 * abs(dv3[i]);
        rgb_error += 2.43131 * abs(dv4[i]);
    }
    return (int)rgb_error;
}
...
static int round_to_nearest_multiple_of_eight(int qp) {
    int tmp1 = clip(qp / 8 * 8);
    int tmp2 = clip((qp / 8 + 1) * 8);
    if (abs(qp - tmp1) < abs(qp - tmp2))
        return tmp1;
    else
        return tmp2;
}

/* the luma qp determination (binary search) process */
static int determine_luma_qp( x264_t *h, int rgb_error_bound, int mb_counter ) {
    if (mb_counter > 0) {
        h->mb.i_qp = 0;
        ...
        h->mb.rgb_error = compute_rgb_error(h);
        // start binary search
        for (int i = 0; i < 10; i++) {
            if (h->mb.rgb_error > rgb_error_bound && h->mb.i_qp > 0) {
                h->mb.rb = h->mb.i_qp;
                h->mb.i_qp = (h->mb.lb + h->mb.i_qp) / 2 > 0 ? (h->mb.lb + h->mb.i_qp) / 2 : 0;
                h->mb.i_chroma_qp = h->chroma_qp_table[h->mb.i_qp];
                //h->mb.i_qp = h->mb.i_qp / 8 * 8;
                // h->mb_encode_flag = 0;
                x264_macroblock_analyse( h );
                // h->mb.i_error = 0;
                h->mb.rgb_error = 0;
            ...
            else if (h->mb.rgb_error < rgb_error_bound && h->mb.i_qp < 51) {
                h->mb.lb = h->mb.i_qp;
                h->mb.i_qp = (h->mb.i_qp + h->mb.rb) / 2 < 51 ? (h->mb.i_qp + h->mb.rb) / 2 : 51;
                h->mb.i_chroma_qp = h->chroma_qp_table[h->mb.i_qp];
                //h->mb.i_qp = h->mb.i_qp / 8 * 8;
                // h->mb_encode_flag = 0;
                x264_macroblock_analyse( h );
        ...
        // h->mb.i_qp = h->mb.i_qp / 8 * 8;
        // h->mb.i_qp = round_to_nearest_multiple_of_eight(h->mb.i_qp);
        // h->mb.i_chroma_qp = h->chroma_qp_table[h->mb.i_qp];
        // x264_macroblock_analyse( h );
    }
...
{
    // 6/30: notice the encoding time for one frame
    // h->tmp_fptr = x264_fopen('/Users/qizhengzhang/Desktop/x264/qp_delta_size_file', 'a');

    // qizheng: read the operation mode here
    // note that if we have many frames, we may have to move these
    // FILE* operation_mode_fptr = fopen("/Users/qizhengzhang/Desktop/x264/operation_mode_file", "r");
    FILE* error_bound_matrix_fptr;
    FILE* designated_qp_matrix_fptr;
    // fscanf(operation_mode_fptr, "%d,", &h->operation_mode_flag);
    if (h->operation_mode_flag == 0) {
        fscanf(h->operation_mode_file, "%d,", &h->universal_qp);
        // fscanf(operation_mode_fptr, "%d,", &h->universal_chroma_qp);
    }
    else if (h->operation_mode_flag == 1) {
        fscanf(h->operation_mode_file, "%d,", &h->universal_error_bound);
        fscanf(h->operation_mode_file, "%d,", &h->universal_error_bound_u);
        fscanf(h->operation_mode_file, "%d,", &h->universal_error_bound_v);
    }
    else if (h->operation_mode_flag == 3) {
        ...
        fscanf(h->operation_mode_file, "%d,", &h->start_qp);
    }
...
#define BS_BAK_ROW_VBV        3
    ...
    // Qizheng: modify slice header qp here!
    if (h->operation_mode_flag == 0) {
        h->sh.i_qp = h->universal_qp;
    }
    else if (h->operation_mode_flag == 1 || h->operation_mode_flag == 2 || h->operation_mode_flag == 3) {
        h->sh.i_qp = 24; // 24 is just a random non-zero qp
    }
    else if (h->operation_mode_flag == 4) {
        h->sh.i_qp = 24; // 24 is just a random non-zero qp
    }
    else if (h->operation_mode_flag == 5) {
        fscanf(h->operation_mode_file, "%d,", &h->sh.i_qp);
    }
    else if (h->operation_mode_flag == 6) {
        h->sh.i_qp = h->start_qp; 
    }
    else if (h->operation_mode_flag == 20) {
        fscanf(h->qp_matrix_file, "%d ", &h->sh.i_qp);
    }
    ...
    /* qizheng: added global fields here */
    int mb_counter = 0;
    int y_error = 0, u_error = 0, v_error = 0;
    while( 1 )
    {
        mb_xy = i_mb_x + i_mb_y * h->mb.i_mb_width;

        // Qizheng: if need to directly modify mb qp, do it here!
        //h->mb.i_qp = h->qp_matrix[mb_xy];
        h->mb.mb_counter = mb_counter;
        ...
        /* universal qp encoding */
        if (h->operation_mode_flag == 10) {
            x264_macroblock_analyse( h );
        }
        else if (h->operation_mode_flag == 0) {
            h->mb.i_qp = h->universal_qp;
            h->mb.i_chroma_qp = h->chroma_qp_table[h->mb.i_qp];
            x264_macroblock_analyse( h );
        }
        else if (h->operation_mode_flag == 1) {
            int error_bound = h->universal_error_bound;
            if (mb_counter > 0) {
                /* luma qp determination process */
                h->mb.i_qp = 0;
                h->mb.lb = 0;
                h->mb.rb = 51;
                // h->mb_encode_flag = 0;
                ...
                // h->mb_encode_flag = 1;
                x264_macroblock_encode( h );
                h->mb.y_error = compute_y_error(h);
                h->mb.u_error = compute_u_error(h);
                h->mb.v_error = compute_v_error(h);
                // start binary search
                for (int i = 0; i < 10; i++) {
                    if (h->mb.y_error > error_bound && h->mb.i_qp > 0) {
                        h->mb.rb = h->mb.i_qp;
                        h->mb.i_qp = (h->mb.lb + h->mb.i_qp) / 2 > 0 ? (h->mb.lb + h->mb.i_qp) / 2 : 0;
                        //h->mb.i_qp = h->mb.i_qp / 8 * 8;
                        // h->mb_encode_flag = 0;
                    ...
                    else if (h->mb.y_error < error_bound && h->mb.i_qp < 51) {
                        h->mb.lb = h->mb.i_qp;
                        h->mb.i_qp = (h->mb.i_qp + h->mb.rb) / 2 < 51 ? (h->mb.i_qp + h->mb.rb) / 2 : 51;
                        //h->mb.i_qp = h->mb.i_qp / 8 * 8;
                        // h->mb_encode_flag = 0;
                ...
                // round to nearest multiple of 8
                //tmp_qp_1 = h->mb.i_qp / 8 * 8 > 51 ? 48 : h->mb.i_qp / 8 * 8;
                //tmp_qp_2 = (h->mb.i_qp / 8 + 1) * 8 > 51 ? 48 : (h->mb.i_qp / 8 + 1) * 8;
                //if (abs(h->mb.i_qp - tmp_qp_1) > abs(h->mb.i_qp - tmp_qp_2))
                //    h->mb.i_qp = tmp_qp_2;
                //else
                //    h->mb.i_qp = tmp_qp_1;
                h->mb.i_qp = h->mb.i_qp / 8 * 8;
                // h->qp_matrix[mb_counter] = h->mb.i_qp;

                /* chroma qp determination process */
                int error_bound_u = h->universal_error_bound_u;
                int error_bound_v = h->universal_error_bound_v;
                h->mb.i_chroma_qp = 0;
                h->mb.lb = 0;
                h->mb.rb = 51;
                // h->mb_encode_flag = 0;
                ...
                // start binary search
                for (int i = 0; i < 10; i++) {
                    if ((h->mb.u_error > error_bound_u || h->mb.v_error > error_bound_v) && h->mb.i_chroma_qp > 0) {
                        h->mb.rb = h->mb.i_chroma_qp;
                        h->mb.i_chroma_qp = (h->mb.lb + h->mb.i_chroma_qp) / 2 > 0 ? (h->mb.lb + h->mb.i_chroma_qp) / 2 : 0;
                        //h->mb.i_qp = h->mb.i_qp / 8 * 8;
                        // h->mb_encode_flag = 0;
                        x264_macroblock_analyse( h );
                        // h->mb.i_error = 0;
                    ...
                    else if (h->mb.u_error < error_bound_u && h->mb.v_error < error_bound_v && h->mb.i_chroma_qp < 51) {
                        h->mb.lb = h->mb.i_chroma_qp;
                        h->mb.i_chroma_qp = (h->mb.i_chroma_qp + h->mb.rb) / 2 < 51 ? (h->mb.i_chroma_qp + h->mb.rb) / 2 : 51;
                        //h->mb.i_qp = h->mb.i_qp / 8 * 8;
                        // h->mb_encode_flag = 0;
                        x264_macroblock_analyse( h );
                        // h->mb.i_error = 0;
                ...
                h->mb.i_qp = 24;
                h->mb.i_chroma_qp = h->chroma_qp_table[h->mb.i_qp]; // added on 7/7
                // x264_log(h, X264_LOG_INFO, "before analysis, qp is %d\n", h->mb.i_qp);
                // h->mb_encode_flag = 0;
                x264_macroblock_analyse( h );
                // x264_log(h, X264_LOG_INFO, "after analysis, qp is %d\n", h->mb.i_qp);
                //h->qp_matrix[mb_counter] = h->mb.i_qp;
            }
        }

        // qizheng: test mode on 7.3
        else if (h->operation_mode_flag == 2) {
            x264_log(h, X264_LOG_INFO, "mb no. %d\n", mb_counter);
            pixel* tmp;
            if (mb_counter > 0) {
                x264_log(h, X264_LOG_INFO, "print u matrix in p_fenc\n");
                tmp = h->mb.pic.p_fenc[1];
                for (int i = 0; i < 8; i++) {
                    for (int j = 0; j < 8; j++) {
                        printf("%d ", tmp[j + i * FENC_STRIDE]);
                    }
                    printf("\n");
                }
                ...
                h->mb_encode_flag = 1;
                x264_macroblock_encode( h );
                x264_log(h, X264_LOG_INFO, "print u matrix when qp = 0\n");
                tmp = h->mb.pic.p_fdec[1];
                for (int i = 0; i < 8; i++) {
                    for (int j = 0; j < 8; j++) {
                        printf("%d ", tmp[j + i * FDEC_STRIDE]);
                    }
                    printf("\n");
                }
                ...
                h->mb_encode_flag = 1;
                x264_macroblock_encode( h );
                x264_log(h, X264_LOG_INFO, "print u matrix when qp = 40\n");
                tmp = h->mb.pic.p_fdec[1];
                for (int i = 0; i < 8; i++) {
                    for (int j = 0; j < 8; j++) {
                        printf("%d ", tmp[j + i * FDEC_STRIDE]);
                ...
                h->mb_encode_flag = 1;
                x264_macroblock_encode( h );
                x264_log(h, X264_LOG_INFO, "print u matrix when qp = 20\n");
                tmp = h->mb.pic.p_fdec[1];
                for (int i = 0; i < 8; i++) {
                    for (int j = 0; j < 8; j++) {
                        printf("%d ", tmp[j + i * FDEC_STRIDE]);
                ...
                h->mb_encode_flag = 1;
                x264_macroblock_encode( h );
                x264_log(h, X264_LOG_INFO, "print u matrix when qp = 10\n");
                tmp = h->mb.pic.p_fdec[1];
                for (int i = 0; i < 8; i++) {
                    for (int j = 0; j < 8; j++) {
                        printf("%d ", tmp[j + i * FDEC_STRIDE]);
        ...
        /* universal error bound encoding */
        else if (h->operation_mode_flag == 3) {
            determine_luma_qp(h, h->universal_error_bound_rgb, mb_counter);
        }

        /* designated error bound encoding */
        else if (h->operation_mode_flag == 4) {
            if (i_mb_x + 1 >= h->mb.i_mb_width) {
                fscanf(error_bound_matrix_fptr, "%d \n", &h->universal_error_bound_rgb);
                
            }
            else {
                fscanf(error_bound_matrix_fptr, "%d ", &h->universal_error_bound_rgb);
            }
            determine_luma_qp(h, h->universal_error_bound_rgb, mb_counter);
        }

        /* designated binary qp encoding */
        else if (h->operation_mode_flag == 5) {
            if (i_mb_x + 1 >= h->mb.i_mb_width) {
                fscanf(designated_qp_matrix_fptr, "%d \n", &h->mb.i_qp);
            }
            else if (mb_counter == 0)
                h->mb.i_qp = h->sh.i_qp;
            else
                fscanf(designated_qp_matrix_fptr, "%d ", &h->mb.i_qp);
            h->mb.i_chroma_qp = h->chroma_qp_table[h->mb.i_qp];
            x264_macroblock_analyse( h );
        }

        /* this mode is for bw test on 7.17 */
        else if (h->operation_mode_flag == 6) {
            int qp_gap = 51 / h->num_groups;
            int group_width = 80 / h->num_groups;
            qp_gap = qp_gap % 2 == 0 ? qp_gap : qp_gap - 1;
            int group_index = i_mb_x / group_width;
            // if (h->num_groups == 16) {
            //     int qps[16] = {1,4,7,10,13,16,19,22,26,29,32,35,38,41,44,47};
            //     h->mb.i_qp = qps[group_index];
            // }
            // else
            h->mb.i_qp = h->start_qp + group_index * qp_gap;
            h->mb.i_chroma_qp = h->chroma_qp_table[h->mb.i_qp];
            x264_macroblock_analyse( h );
        }

        /* multi-frame QP designation mode */
        else if (h->operation_mode_flag == 20) {
            if (i_mb_x + 1 >= h->mb.i_mb_width) {
        ...
        // qizheng: new version on 6/19, make sure error bound is satisfied
        /*
        x264_log(h, X264_LOG_INFO, "encoding mb %d\n", mb_counter);
        if (mb_counter > 0 && i_mb_y % 3 == 0 && i_mb_x % 4 == 0) {
        //if (mb_counter > 0) {
            h->mb.i_qp = 0;
            h->mb.lb = 0;
            h->mb.rb = 51;
            //h->mb.i_error = 0;
            x264_log(h, X264_LOG_INFO, "before analyse, h->mb.i_error is %d\n", h->mb.i_error);
            x264_macroblock_analyse( h );
            x264_log(h, X264_LOG_INFO, "after analyse, h->mb.i_error is %d\n", h->mb.i_error);
            for (int i = 0; i < 5; i++) {
                x264_log(h, X264_LOG_INFO, "in the loop, h->mb.i_error is %d\n", h->mb.i_error);
                if (h->mb.i_error > h->error_bound_matrix[mb_counter] && h->mb.i_qp > 0) {
                    h->mb.rb = h->mb.i_qp;
                    h->mb.i_qp = (h->mb.lb + h->mb.i_qp) / 2 > 0 ? (h->mb.lb + h->mb.i_qp) / 2 : 0;
                    h->mb.i_qp = h->mb.i_qp / 8 * 8;
                    //h->mb.i_error = 0;
                ...
                else if (h->mb.i_error < h->error_bound_matrix[mb_counter] && h->mb.i_qp < 51) {
                    h->mb.lb = h->mb.i_qp;
                    h->mb.i_qp = (h->mb.i_qp + h->mb.rb) / 2 < 51 ? (h->mb.i_qp + h->mb.rb) / 2 : 51;
                    h->mb.i_qp = h->mb.i_qp / 8 * 8;
                    //h->mb.i_error = 0;
            ...
            h->qp_matrix[mb_counter] = h->mb.i_qp;
            x264_log(h, X264_LOG_INFO, "before leaving if, h->mb.i_error is %d\n", h->mb.i_error);
            h->error_matrix[mb_counter] = h->mb.i_error;
        }
        else if (mb_counter == 0) {
            h->mb.i_qp = 24;
            //h->mb.i_error = 0;
            ...
            h->qp_matrix[mb_counter] = h->mb.i_qp;
            h->error_matrix[mb_counter] = h->mb.i_error;
            //h->qp_matrix[mb_xy] = h->mb.i_error;
        }
        else {
            int x_coor = i_mb_x / 4 * 4;
            int y_coor = i_mb_y / 3 * 3;
            h->mb.i_qp = h->qp_matrix[y_coor*80 + x_coor];
            //h->mb.i_error = 0;
            x264_log(h, X264_LOG_INFO, "before analyse, h->mb.i_error is %d\n", h->mb.i_error);
            x264_macroblock_analyse( h );
            x264_log(h, X264_LOG_INFO, "after analyse, h->mb.i_error is %d\n", h->mb.i_error);
        ...
        */
        ...
        // 6/20 version of qp search: now error computation is correct
        
        //x264_log(h, X264_LOG_INFO, "Encoding mb %d\n", mb_counter);
        //x264_log(h, X264_LOG_INFO, "QP_MAX_SPEC is: %d\n", QP_MAX_SPEC);
        //x264_log(h, X264_LOG_INFO, "in rc, i_qp_min is: %d\n", h->param.rc.i_qp_min);
        //x264_log(h, X264_LOG_INFO, "in rc, i_qp_max is: %d\n", h->param.rc.i_qp_max);
        /*
        if (mb_counter > 0) {
            h->mb.i_qp = 0;
            h->mb.lb = 0;
            ...
            // x264_log(h, X264_LOG_INFO, "after analysis, qp is %d\n", h->mb.i_qp);
            h->mb.i_error = 0;
            //x264_log(h, X264_LOG_INFO, "before encode, qp is %d\n", h->mb.i_qp);
            h->mb_encode_flag = 1;
            x264_macroblock_encode( h );
            // x264_log(h, X264_LOG_INFO, "after encode, qp is %d\n", h->mb.i_qp);
            // start binary search
            for (int i = 0; i < 5; i++) {
                if (h->mb.i_error > h->error_bound_matrix[mb_counter] && h->mb.i_qp > 0) {
                    h->mb.rb = h->mb.i_qp;
...
reencode:
        //x264_log(h, X264_LOG_INFO, "before encode, h->mb.i_error is %d\n", h->mb.i_error);
        h->mb.i_error = 0;
        //x264_log(h, X264_LOG_INFO, "before encode, qp is %d\n", h->mb.i_qp);
        //h->mb_encode_flag = 1;
        x264_macroblock_encode( h );
        // x264_log(h, X264_LOG_INFO, "after encode, chroma qp is: %d\n", h->mb.i_chroma_qp);
        // h->mb.y_error = compute_y_error(h);
        // h->mb.u_error = compute_u_error(h);
        // h->mb.v_error = compute_v_error(h);
        h->mb.rgb_error = compute_rgb_error(h);
        // x264_log(h, X264_LOG_INFO, "after encode, qp is %d\n", h->mb.i_qp);
        // h->error_matrix[mb_counter] = h->mb.i_error;
        // h->chroma_qp_matrix[mb_counter] = h->mb.i_chroma_qp;
        // h->qp_matrix[mb_counter] = h->mb.i_qp;
        //x264_log(h, X264_LOG_INFO, "after encode, h->mb.i_error is %d\n", h->mb.i_error);

        // if (mb_counter == 30)
        //     exit(0);

        /* qizheng: write empirical yuv error to files */
        // fprintf(empirical_y_error_fptr, "%d ", h->mb.y_error);
        // if (i_mb_x + 1 >= h->mb.i_mb_width) {
        //     fprintf(empirical_y_error_fptr, "\n");
        // }

        // fprintf(empirical_u_error_fptr, "%d ", h->mb.u_error);
        // if (i_mb_x + 1 >= h->mb.i_mb_width) {
        //     fprintf(empirical_u_error_fptr, "\n");
        // }

        // fprintf(empirical_v_error_fptr, "%d ", h->mb.v_error);
        // if (i_mb_x + 1 >= h->mb.i_mb_width) {
        //     fprintf(empirical_v_error_fptr, "\n");
        // }
        ...
        // if (i_mb_x + 1 >= h->mb.i_mb_width) {
        //     fprintf(qp_delta_size_fptr, "\n");
        // }

        /* qizheng: print out u and v matrices to see if error computation is correct */
        // pixel *p_dst;
        // pixel *p_src;
        // x264_log(h, X264_LOG_INFO, "chroma qp is now: %d\n", h->mb.i_chroma_qp);
        // x264_log(h, X264_LOG_INFO, "u_error is now: %d\n", h->mb.u_error);
        // x264_log(h, X264_LOG_INFO, "print out u of p_src for mb no. %d\n", mb_counter);
        // p_src = h->mb.pic.p_fenc[1];
        // for (int i = 0; i < 8; i++) {
        //     for (int j = 0; j < 8; j++) {
        //         printf("%d ", p_src[j + i * FENC_STRIDE]);
        //     }
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "print out u of p_dst for mb no. %d\n", mb_counter);
        // p_dst = h->mb.pic.p_fdec[1];
        // for (int i = 0; i < 8; i++) {
        //     for (int j = 0; j < 8; j++) {
        //         printf("%d ", p_dst[j + i * FDEC_STRIDE]);
        //     }
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "v_error is now: %d\n", h->mb.v_error);
        // x264_log(h, X264_LOG_INFO, "print out v of p_src for mb no. %d\n", mb_counter);
        // p_src = h->mb.pic.p_fenc[2];
        // for (int i = 0; i < 8; i++) {
        //     for (int j = 0; j < 8; j++) {
        //         printf("%d ", p_src[j + i * FENC_STRIDE]);
        ...
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "print out v of p_dst for mb no. %d\n", mb_counter);
        // p_dst = h->mb.pic.p_fdec[2];
        // for (int i = 0; i < 8; i++) {
        //     for (int j = 0; j < 8; j++) {
        //         printf("%d ", p_dst[j + i * FDEC_STRIDE]);
        ...
        pixel *p_src;
        // x264_log(h, X264_LOG_INFO, "mb no. %d\n", mb_counter);
        // x264_log(h, X264_LOG_INFO, "printing original Y channel\n");
        ...
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "end original Y channel\n");
        // x264_log(h, X264_LOG_INFO, "printing reconstructed Y channel\n");
        ...
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "end reconstructed Y channel\n");

        // x264_log(h, X264_LOG_INFO, "printing original U channel\n");
        // p_src = h->mb.pic.p_fenc[1];
        // for (int i = 0; i < 8; i++) {
        //     for (int j = 0; j < 8; j++) {
        //         printf("%d ", p_src[j + i * FENC_STRIDE]);
        ...
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "end original U channel\n");
        // x264_log(h, X264_LOG_INFO, "printing reconstructed U channel\n");
        // p_dst = h->mb.pic.p_fdec[1];
        // for (int i = 0; i < 8; i++) {
        //     for (int j = 0; j < 8; j++) {
        //         printf("%d ", p_dst[j + i * FDEC_STRIDE]);
        ...
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "end reconstructed U channel\n");

        // x264_log(h, X264_LOG_INFO, "printing original V channel\n");
        // p_src = h->mb.pic.p_fenc[2];
        // for (int i = 0; i < 8; i++) {
        //     for (int j = 0; j < 8; j++) {
        //         printf("%d ", p_src[j + i * FENC_STRIDE]);
        ...
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "end original V channel\n");
        // x264_log(h, X264_LOG_INFO, "printing reconstructed V channel\n");
        // p_dst = h->mb.pic.p_fdec[2];
        // for (int i = 0; i < 8; i++) {
        //     for (int j = 0; j < 8; j++) {
        //         printf("%d ", p_dst[j + i * FDEC_STRIDE]);
        ...
        //     printf("\n");
        // }
        // x264_log(h, X264_LOG_INFO, "end reconstructed V channel\n");

        /* store p_src and p_dst in global matrices */
        // int x, y;
        // p_dst = h->mb.pic.p_fdec[0];
        // for (int i = 0; i < 16; i++) {
        //     for (int j = 0; j < 16; j++) {
        //         x = mb_counter % 80;
        //         y = mb_counter / 80;
        //         h->y_matrix[y*1280+x*16+] = p_dst[j + i * FDEC_STRIDE];
        //     }
        // }
        ...
        // qizheng: print out mb size here
        //h->mb_size_matrix[mb_counter] = mb_size;
        //if (mb_counter == 304 || mb_counter == 2859) {
        //    x264_log(h, X264_LOG_INFO, "mb_counter is: %d (end)\n", mb_counter);
        ...
        // x264_log(h, X264_LOG_INFO, "h->mb.i_qp is: %d\n", h->mb.i_qp);
        // x264_log(h, X264_LOG_INFO, "h->sh.i_last_mb is: %d\n", h->sh.i_last_mb);
...
cont:
        ...
        // Qizheng: increment the counter here
        mb_counter++;
    }
    ...
    // Qizheng: free the memory (otherwise may abort)
    //printf("reach here5\n");
    //x264_free(h->qp_matrix);
    //printf("reach here6\n");

    // Qizheng: print the empirical qp values
    /*
    for (int i = 0; i < 45; i++) {
        for (int j = 0; j < 80; j++) {
            fprintf(empirical_qp_matrix_fptr, "%d ", h->qp_matrix[i*80+j]);
            fprintf(empirical_chroma_qp_matrix_fptr, "%d ", h->chroma_qp_matrix[i*80+j]);
            // fprintf(empirical_error_matrix_fptr, "%d ", h->error_matrix[i*80+j]);
            //fprintf(mb_size_matrix_fptr, "%d ", h->mb_size_matrix[i*80+j]);
        }
        fprintf(empirical_qp_matrix_fptr, "\n");
        fprintf(empirical_chroma_qp_matrix_fptr, "\n");
        // fprintf(empirical_error_matrix_fptr, "\n");
        //fprintf(mb_size_matrix_fptr, "\n");
    }
    //x264_free(h->qp_matrix);
    //x264_free(h->error_matrix);
    //x264_free(h->mb_size_matrix);
    */
    return 0;
}
