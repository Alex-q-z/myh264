...
static void cabac_qp_delta( x264_t *h, x264_cabac_t *cb )
{
    ...
    // x264_log(h, X264_LOG_INFO, "in cabac_qp_delta, h->mb.i_qp is: %d\n", h->mb.i_qp);
    // x264_log(h, X264_LOG_INFO, "in cabac_qp_delta, h->mb.i_last_qp is: %d\n", h->mb.i_last_qp);
    // x264_log(h, X264_LOG_INFO, "in cabac_qp_delta, i_dqp is: %d\n", i_dqp);
    ...
    /* qizheng: add size checkpoint here */
    // int size_before = cb->f8_bits_encoded;
    if( i_dqp != 0 )
    {
    ...
    // fprintf(qp_delta_size_fptr, "%d ", cb->f8_bits_encoded - size_before);
    // x264_log(h, X264_LOG_INFO, "qp_delta size is: %d\n", cb->f8_bits_encoded - size_before);
}
...
static ALWAYS_INLINE void macroblock_write_cabac_internal( x264_t *h, x264_cabac_t *cb, int plane_count, int chroma )
{
    ...
    if( h->mb.i_cbp_luma || (chroma && h->mb.i_cbp_chroma) || i_mb_type == I_16x16 )
    {
        ...
        // qizheng: delete qp encoding
        // int before_size = cb->f8_bits_encoded;
        cabac_qp_delta( h, cb );
        // x264_log(h, X264_LOG_INFO, "size is: %d\n", cb->f8_bits_encoded - before_size);
        // fprintf(h->tmp_fptr, "%d ", cb->f8_bits_encoded - before_size);
